...
class DilatedParallelConvBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        d3 = d2 + d3
        d4 = d3 + d4
        att = jittor.sigmoid(self.attention(jittor.cat([d1, d2, d3, d4], 1)))
        d1 = d1 + d1 * att[:, 0].unsqueeze(1)
        d2 = d2 + d2 * att[:, 1].unsqueeze(1)
        d3 = d3 + d3 * att[:, 2].unsqueeze(1)
        d4 = d4 + d4 * att[:, 3].unsqueeze(1)
        output = self.conv1x1_fuse(jittor.cat([d1, d2, d3, d4], 1))
...
class MiniSeg(nn.Module):
    ...
    def forward(self, input):
        ...
        output1_cat = self.cat1(jittor.cat([long1, output1], 1))
        ...
        output2_cat = self.cat2(jittor.cat([long2, output2], 1))
        ...
        output3_cat = self.cat3(jittor.cat([long3, output3], 1))
