...
from jittor import utils
from hparam import hparams as hp
from utils.metric import metric
from jittor.optim.lr_scheduler import ReduceLROnPlateau, StepLR, CosineAnnealingLR
...
device = jittor.device("cuda:0" if jittor.cuda.is_available() else "cpu")
...
def train():
    parser = argparse.ArgumentParser(description='Pyjittor Medical Segmentation Training')
    parser = parse_training_args(parser)
    args, _ = parser.parse_known_args()

    args = parser.parse_args()

    jittor.backends.cudnn.deterministic = True
    jittor.backends.cudnn.enabled = args.cudnn_enabled
    jittor.backends.cudnn.benchmark = args.cudnn_benchmark
    ...
    model = jittor.nn.DataParallel(model, device_ids=devicess)
    from jittorsummary import summary
    print('params', summary(model, input_size=(1, 128, 128, 128)))
    inputs = jittor.randn(1, 1, 128, 128, 128).to(jittor.float32)
    from thop import profile
    flops, params = profile(model, inputs=(inputs.to(device),))
    print("FLOPs=", str(flops / 1e9) + '{}'.format("G"))
    print("params=", str(params / 1e6) + '{}'.format("M"))
    optimizer = jittor.optim.Adam(model.parameters(), lr=args.init_lr)
    ...
    if args.ckpt is not None:
        ...
        ckpt = jittor.load(os.path.join(args.output_dir, args.latest_checkpoint_file),
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            #半径
            if jittor.sum(y_lab) > 0:
                posmask = y_lab.numpy().astype(np.bool_)
                if posmask.any():
                    y_dist = jittor.tensor(distance(posmask).astype('int16'))
                ...
                #数据送入cuda
                x = x.type(jittor.FloatTensor).cuda()
                y_lab = y_lab.type(jittor.FloatTensor).cuda()
                y_dist = y_dist.type(jittor.FloatTensor).cuda()
                y_ori = y_ori.type(jittor.FloatTensor).cuda()
                seg,dist,oriention = model(x)
                # for metrics
                map_logits = jittor.sigmoid(seg)
        ...
        if epoch % args.epochs_per_checkpoint == 0:
            ...
            # ckpt = jittor.load(os.path.join(train_path, args.latest_checkpoint_file),
            #                   map_location=lambda storage, loc: storage)
            ...
            for i, subj in enumerate(test_dataset.subjects):
                ...
                patch_loader = jittor.utils.data.DataLoader(grid_sampler, batch_size=2)
...
def test():
    parser = argparse.ArgumentParser(description='Pyjittor Medical Segmentation Testing')
    parser = parse_training_args(parser)
    args, _ = parser.parse_known_args()
    ...
    model = jittor.nn.DataParallel(model, device_ids=devicess, output_device=[1])
    ...
    ckpt = jittor.load(os.path.join(train_path,'checkpoint_0090.pt'), map_location=lambda storage, loc: storage)
    # ckpt = jittor.load(train_path, map_location=lambda storage, loc: storage)
    ...
    for i, subj in enumerate(test_dataset.subjects):
        ...
        with jittor.no_grad():
            for patches_batch in patch_loader:
                ...
                dist_labels = jittor.argmax(dist_labels, dim=1)
                dist_labels = dist_labels.unsqueeze(1)
                #oriention
                ori_labels = oriention.clone()
                # ori_labels = jittor.argmax(ori_labels, dim=1)
